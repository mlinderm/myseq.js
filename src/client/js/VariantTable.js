...
import { ExpandRow } from 'react-bootstrap-table';
    ...
    return (
      <tbody>
        <tr style={(this.state.rowHidden === true) ?
          ...
          <td onClick={()=>this.handleRowClick()}>{this.props.variantProp.genotype()}</td>
        </tr>
        {
            ...
            ? <tr>
                <td colSpan="3">
                  <ul>
                    ...
                    <li>
                      SNPedia: {rsidsForMapping.map(id=>(
                        (id !== ".") ? (
                          <a
